...
export default function RelationshipExplorer() {
  ...
  return (
    <div className="sql-pg">
      ...
      {kind === '1:1' && (
        <>
          <p style={{ fontSize: 14, color: '#334155' }}>One row matches exactly one row. A user has one profile. The foreign key lives on either side and is marked UNIQUE so it can only match once.</p>
      ...
      {kind === '1:many' && (
        <>
          <p style={{ fontSize: 14, color: '#334155' }}>One customer has many orders, but each order has one customer. The &quot;many&quot; side carries the foreign key.</p>
      ...
      {kind === 'many:many' && (
        <>
          <p style={{ fontSize: 14, color: '#334155' }}>Students take many courses; each course has many students. A junction table records one row per pairing.</p>
